...
export async function POST(request: Request){
    ...
    try {
        ...
        if(existingUserByEmail) {
            if (existingUserByEmail.isVerified) {
                ...
                existingUserByEmail.verifyCodeExpiry = new Date(Date.now() + 3600000) // error
    ...
    if(!emailResponse.success) {
     ...
     return Response.json({
        ...
        console.error('Erroe registering user', error)  // show in terinmal
        return Response.json(  // ye frontened pe jayega
            {
                success: false,
